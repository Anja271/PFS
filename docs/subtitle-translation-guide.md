...
- Preserve every independently spoken Korean discourse marker `자` as the romanized sound `ja`. This is Bamby's established MC running gag, and the other members often imitate or repeat it to tease him. Do not replace it with generic English filler such as `all right`, `now`, or `okay`; retain the number and rhythm (`자, 자` → `ja, ja...`) regardless of which member voices the imitation. Do not apply this rule when `자` is merely a syllable inside another Korean word.
- Preserve deliberate callback verbs coined with `-hamin` or a similar `-min` ending as recognizable romanized wordplay. Give a concise English gloss at the first occurrence (`neomeoga-min--move on`) and retain later teasing repetitions. Do not normalize the invented verb into ordinary English, but do not misclassify normal grammar or an ordinary mention of Hamin as wordplay; require the surrounding reaction or repetition to establish the joke.
- Preserve Yejun's coined catch-all verb `duduhanda` and all later conjugations or ASR variants with visible `dudu` in English (`duduhanda`, `dudu-ing`, `to dudu`). Its situational meaning is determined by tone and context: translate or gloss a supported function such as sulking, scolding, threatening, pleading, or promising while retaining `dudu`; when the intended function is unclear, leave the form productively ambiguous rather than choosing one. Distinguish the verb from a person/game handle named Dudu through context, but preserve the spoken `Dudu` in either case.
- Preserve Bamby/Bonggu's coined `Bong-` catchphrase family and later references as distinct visible romanizations. Use `Bonguk` and `Bongus` according to the supported spoken variant and explain their first introduction as Bonggu-based greeting/catchphrase forms. Preserve further transparent formations such as `Bong-kkeut` with a short gloss (`kkeut` = `end`). Do not silently normalize every ASR variant into one form or replace the family with a generic greeting.
